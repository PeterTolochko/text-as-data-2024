...
The challenge today is a repetition of the basics of R and RStudio. Credit goes to Philipp Masur who designed most of the script. 

Please go though the following script and execute the code.
There are some small tasks labeled **Exercise:**. Please complete them until the coding session tomorrow. If you are familiar with the basics of R, this will be possible to solve in no time. Feel free to help others who need support. If you need a refresher to the R basics, this is the invitation. 

We will look at the answers in the coding session tomorrow.
...
The R programming language does come with its own user interface which in itself is adequate for modest applications. There are many useful editors available, but at this time, RStudio is by far the most popular one. RStudio provides many convenient features and tools and makes working with R a lot easier. 

By default, if you opened this file you are currently seeing four cells (The order of those cells can be changed via Tools > Global Options > Pane Layout). 

1. Lower left: **Console**. This window is a classical command-line interface. This means that it is constantly attentive to you commands. We can type any R code chunk after the prompt-sign ">" and run it directly with "Enter". We will use this cell mostly as our "Output-Window". There is also the option to use the RStudio terminal (next to the console tab), after being activated it is possible to execute files in the terminal directly from within RStudio (see: https://support.posit.co/hc/en-us/articles/115010737148-Using-the-RStudio-Terminal-in-the-RStudio-IDE).
...
It is important to understand that R is nothing but a very powerful calculator. The most basic code is hence a simple maths operation. Of course, we can use parantheses as well. 
